...
type keyHeader struct {
    AuthApiKey string `header:"ApiKey"`
}

func New(apiKey string) *PocketAPI {
    api := PocketAPI{apiKey: apiKey};
...
func (api *PocketAPI) registerApi() {
    ...
    apiGroup.Use(authApiKey(api.apiKey))
    apiGroup.POST("", Create(api.Db))
    apiGroup.POST("/:id/upload", Upload(api.Db))
}

func (api *PocketAPI) registerPublicApi() {
    apiGroup := api.Router.Group("/api")
